...
query = "(SELECT TO_CHAR(SRC_DT_TRX,'YYYYMMDD') ID_DATE, SRC_DS_LOCATION,round(count(SRC_NR_CSTMR_MSISDN),0) count FROM IC_STAGE.SRC_880_TRX_DTLS WHERE TO_CHAR(SRC_DT_TRX,'YYYYMMDD') = '20190317' group by TO_CHAR(SRC_DT_TRX,'YYYYMMDD') , SRC_DS_LOCATION)  Table_1" 


query2 = "(select /*+parallel*/ TO_CHAR(SRC_DT_TRX,'YYYYMMDD') ID_DATE,SRC_DS_CATEGORY DS_CATEGORY,SRC_DS_LOCATION DS_LOCATION,COUNT(*) COUNT from ic_stage.SRC_880_TRX_DTLS where  TO_CHAR(SRC_DT_TRX,'YYYYMMDD') >=20190201 and  SRC_DS_CATEGORY='Blaze' AND SRC_NR_CSTMR_MSISDN IS NOT NULL GROUP BY SRC_DS_LOCATION,TO_CHAR(SRC_DT_TRX,'YYYYMMDD'),SRC_DS_CATEGORY,SRC_DS_LOCATION) Table_2"
